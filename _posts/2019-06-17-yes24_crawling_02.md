...
02-2) 현재 활성화되어 있는 도서 페이지의 주소를 ```url``` 변수에 할당해줍니다. requests와 BeautifulSoup4 라이브러리의 다음 메소드들로 ```url```에 해당하는 웹 사이트의 HTML 정보를 읽어옵니다. 이때, 위에서 찾은 책 제목에 해당하는 태그의 위치를 ```tag_name``` 변수에 할당하고, ```soup.select```를 이용하여 해당 태그만 선택하여 가져옵니다.
...
02-3) ```books```변수의 값을 출력하면 다음과 같이 책 제목이 위치한 태그를 출력해줍니다.
